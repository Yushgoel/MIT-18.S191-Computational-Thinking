...
# ╔═╡ 24e03ffc-48c5-11eb-00d9-770b8d68de07
ex_1_scene[5]
...
reflect(philip.l, ex_1_scene[3].normal), philip.l
...
function interact(photon::Photon, hit::Intersection{Wall})
	new_photon = Photon(hit.point, reflect(photon.l, hit.object.normal), photon.ior)
	return new_photon
end
...
function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
	...
	elseif discriminant > ϵ
		...
		if root_minus < 0
			...
			t = minimum([root_plus, root_minus])			
		end
	...
	point = photon.p + photon.l .* t
	distance = norm(point - photon.p)
	return Intersection(sphere, distance, point)
end
...
function closest_hit(photon::Photon, objects::Vector{<:Object})
	...
	for object in objects
		push!(intersections, intersection(photon, object))
	end
...
# ╔═╡ 8ac775d2-48c9-11eb-205f-f1b3024aa54f
philip.p
...
function interact(photon::Photon, hit::Intersection{Sphere})
	...
	new_photon = Photon(hit.point + correction, new_l, new_ior)
...
function trace(photon::Photon, scene::Vector{<:Object}, N)
	...
	for i in 1:N
		push!(photons, step_ray(photons[length(photons)], scene))
	end
...
# ╔═╡ 0c5eca3a-48d5-11eb-03d0-3d7716757503
test_photons = [Photon([0,1], [1,0], 1.0), Photon([0,0.5], [1,0], 1.0), Photon([0,0], [1,0], 1.0), Photon([0,-0.5], [1,0], 1.0), Photon([0,-1], [1,0], 1.0), Photon([0,-1.5], [1,0], 1.0),  Photon([0,-2], [1,0], 1.0), Photon([0,-2.5], [1,0], 1.0),  Photon([0,-3], [1,0], 1.0),  Photon([0,-3.5], [1,0], 1.0), Photon([0,-4], [1,0], 1.0)]
...
let
	...
	for test_lens_photon in test_photons
		path = accumulate(1:sphere_test_ray_N; init=test_lens_photon) do old_photon, i
			step_ray(old_photon, scene)
		end
