...
function collide_boundary(c::Coordinate, L::Number)
	...
	return Coordinate(bound_x, bound_y)
end
...
function trajectory(c::Coordinate, n::Int, L::Number)
	traj = accumulate(+, rand(possible_moves, n); init=c)
	traj = collide_boundary.(traj, L)
	return traj
end
...
let
	...
	for i in 1:10
		traj = trajectory(Coordinate(0, 0), 1000)
		plot_trajectory!(p, traj)
	end
	p
end

# ╔═╡ 0295483a-4444-11eb-27d9-090c40cd7bf9
trajectory(Coordinate(0, 0), 10, 2)
...
# ╔═╡ f54544bc-444e-11eb-0f8f-2b3c16bdb63b
...
does_not_cause_outbreak = CollisionInfectionRecovery(0.5, 0.001) #Default values don't cause outbreak
...
# ╔═╡ c19218bc-44ff-11eb-021f-5738fccfacbd
...
function initialize(N::Number, L::Number)
	agents = Vector{Agent}()
	for i in 1:N
		agent = Agent(Coordinate(rand(-L:L), rand(-L:L)), S)
		push!(agents, agent)
	end
...
# ╔═╡ d5184924-444b-11eb-1614-93f60bdd83ce
...
function visualize(agents::Vector, L)
	...
	for i in 1:length(agents)
		scatter!(p, [agents[i].position.x], [agents[i].position.y], c = [color(agents[i])], legend=false)
	end
...
function initialize_social(N, L)
	agents = []
	scores = LinRange(0.1, 0.5, N)
	for i in 1:N
		agent = SocialAgent(Coordinate(rand(-L:L), rand(-L:L)), S, 0, scores[i])
		push!(agents, agent)
	end
...
let
	...
	for i in 1:k_sweep_max
		S_count, I_count, R_count = count_statuses(agents)
	...
	p = plot()
	plot!(p, 1:k_sweep_max, S_counts, color="blue")
	plot!(p, 1:k_sweep_max, I_counts, color="red")
	plot!(p, 1:k_sweep_max, R_counts, color="green")
	p
end
...
let
    ...
    @gif for t in 1:Tmax
		...
		S_count, I_count, R_count = count_statuses(x)
		push!(Ss, S_count)
		push!(Is, I_count)
		push!(Rs, R_count)
...
let
	...
	for i in 1:k_sweep_max
		...
		for j in 1:N
			step!(agents, L, does_not_cause_outbreak)#causes_outbreak)
		end
	end
	# compute k_sweep_max number of sweeps and plot the SIR
	p = plot()
...
let
	...
	Ss, Is, Rs = [], [], []
	Tmax = 200
	pandemic = CollisionInfectionRecovery(0.1, 0.05)
	...
	@gif for t in 1:Tmax
		...
		S_count, I_count, R_count = count_statuses(social_agents)
		push!(Ss, S_count)
		push!(Is, I_count)
		push!(Rs, R_count)
...
let
	global social_agents
	scores = []
	num_infecteds = []
	
	for i in 1:length(social_agents)
		push!(scores, social_agents[i].social_score)
		push!(num_infecteds, social_agents[i].num_infected)
	end
	scatter(scores, num_infecteds)
end
