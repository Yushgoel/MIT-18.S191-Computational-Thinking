...
# ╔═╡ df75cf0e-499b-11eb-32f4-752f71f4e473
...
# ╔═╡ c61a3ec6-4b1e-11eb-0ac3-a75c0f613577
...
begin
	function interact(ray::Photon, hit::Intersection{Sphere}, num_intersections::Integer, objects::Vector{O}) where {O <: Object}
		...
		if hit.object.s.r != 0
			reflect_ray = Photon(hit.point, reflect(ray.l, n), ray.c, ray.ior)
			
			reflect_ray = Photon(hit.point, reflect(ray.l, n), step_ray(reflect_ray, objects, num_intersections-1).c, ray.ior)
		end
		...
		if hit.object.s.t != 0
			...
			new_l = refract(ray.l, n, ray.ior, new_ior)
			refract_ray = Photon(hit.point, new_l, ray.c, new_ior)

			refract_ray = Photon(hit.point, new_l, step_ray(refract_ray, objects, num_intersections-1).c, new_ior)
		end

		final_color = reflect_ray.c * hit.object.s.r + refract_ray.c * hit.object.s.t + hit.object.s.c.alpha * hit.object.s.c

		return Photon(ray.p, zeros(length(ray.l)), final_color, ray.ior)
...
function padded(img)
	...
	for i in 1:h
		for j in 1:3w
			...
			if j in w:2w
				# padded_img[i, j] = img[i - h_05 + 1, j - w_05 + 1]
				missing
			else
				padded_img[(j-1) * h + i] = RGB{Float64}(0.25, 0.3, 0.9)
			end
		end
	end
