...
# ╔═╡ 3bb09036-4b60-11eb-2f30-27aaaa430777
...
∆t - Smaller ∆t gives smoother simulation. 
∆x - Smaller ∆x gives more accurate heat transfer model
"""
...
Everything suddenly becomes extremely hot.
...
function absorbed_solar_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	...
	for i in 1:size(T)[1]
		lat_S = S_at(y[i]; grid= model.grid, S_mean = model.params.S_mean)
		lat_S /= 60*60*24*365.25
		
		l1 = (1 .- alpha[i, :])
		l2 = lat_S .* l1
		l3 = l2 ./ (4 * model.params.C)
		
		T[i, :] = l3#lat_S .* (1 .- alpha[i, :]) ./ 4
	end
...
function outgoing_thermal_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	...
	for i in size(T)[1]
		for j in size(T)[2]
			new_T[i, j] = outgoing_thermal_radiation(T[i, j]; C=model.params.C, A = model.params.A, B = model.params.B)
		end
	end
	return new_T
end
...
function timestep!(sim::ClimateModelSimulation{RadiationOceanModel})
	...
	# power = (floor.(log.(10, abs.(tendencies)))).^10
	# tendencies = tendencies ./ power

	# println(findmax(outgoing_thermal_radiation(sim.T, sim.model)))
	# println(findmin(outgoing_thermal_radiation(sim.T, sim.model)))
	sim.T .+= sim.Δt*tendencies
	println(findmax(sim.T))
	println(findmin(sim.T))
	sim.iteration += 1
end

# ╔═╡ 126bffce-2d0b-11eb-2bfd-bb5d1ad1169b
function runtime(N)
	runtime = @elapsed for i in 1:100
		default_grid = Grid(N, 6000.0e3);
		ocean_velocities = DoubleGyre(default_grid);
		ocean_T_init = linearT(default_grid); 
...
# ╔═╡ a9295dcc-4c03-11eb-3176-e77fd12e5460
length(radiation_sim.model.grid.y[:])

# ╔═╡ ab76d9fc-4c03-11eb-365f-4bea433d35c4
radiation_sim.model.params.S_mean

# ╔═╡ af31b4da-4c04-11eb-388a-abc235fe1d6e
radiation_sim.T
