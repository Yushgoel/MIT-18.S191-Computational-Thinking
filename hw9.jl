...
# ╔═╡ f61431a8-4a75-11eb-18b8-596cc8ba12e2
...
Less effect of doubling CO2.
Higher value leads to more positive effect of Co2 on T
"""
...
# ╔═╡ 424d968c-4a66-11eb-01c1-a56a26ed2764
B_range = -2.5:.001:0
...
histogram(ECS_samples, size=(600, 250), label=nothing, xlabel = "ECS samples", ylabel = "samples")
...
# ╔═╡ 9454f33a-4a75-11eb-011a-c75f310edf69
sum(ECS_samples .> ECS(;B=mean(B_samples))) / length(ECS_samples .> ECS(;B=mean(B_samples)))
...
worse, since probability of going higher than the estimation with mean B is >50%. But only slightly
"""
...
# ╔═╡ ea9e8aca-4a76-11eb-1744-f782a7f3ab25
plot(simulated_rcp85_model.t, simulated_rcp85_model.T)

# ╔═╡ 0824758c-4a77-11eb-1d23-418a4498785e
md"""
Temperature at 2100 is 17.5676
"""
...
function temperature_response(CO2::Function, B::Float64=-1.3)
	...
	ebm = Model.EBM(14.0, 1850, 1, t -> CO2(t), B=B)
	Model.run!(ebm, 2100)
	return ebm.T[length(ebm.T)]
end

# ╔═╡ 014e05fe-4a7a-11eb-3ed3-695c0c730851
5
...
expected_double_CO2_year = let
	findfirst(Model.CO2_RCP85.(t) .>= 560) + 1850
end
...
sum((.-temperature_response.(Model.CO2_RCP85, B_samples)) .> 16) / length((.-temperature_response.(Model.CO2_RCP85, B_samples)) .> 16)
...
# ╔═╡ 59e2c91c-4a79-11eb-3566-a3d5b41e6244
@bind CO2_log Slider(log(10, CO2min):0.01:log(10, CO2max), show_value=true)

# ╔═╡ 88249c3c-4a7a-11eb-23ac-71ff4dfce4b8
CO2 = 10^CO2_log
...
ebm = Model.EBM(Tneo, 0., 5., Model.CO2_const)#CO2_const)
...
# ╔═╡ d4250eb2-4b26-11eb-1269-292bc4cf70b2
...
# ╔═╡ 79a3b7c6-4b27-11eb-0de9-8f28f221d817
function get_answer()
	breakpoint_temp = 10 #From graph
	for init_CO2 in 100:1_000_000
		if equilibrium_temperature(init_CO2) > breakpoint_temp
			return init_CO2
		end
	end
end
