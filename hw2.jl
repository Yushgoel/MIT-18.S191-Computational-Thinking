...
342, reason being that data is no longer being coppied with vcat, but directly assigned.
"""
...
function remove_in_each_row_views(img, column_numbers)
	...
	for (i, j) in enumerate(column_numbers)
		# EDIT THE FOLLOWING LINE and split it into two lines
		# to avoid using `vcat`.
		@views img′[i, 1:j-1] = img[i, 1:j-1]
		@views img′[i, j:end] = img[i, j+1:end]
	end
	img′
end
...
1026 allocations, reason being that views doesn't copy any data at all, so no redundant numbers
"""
...
function greedy_seam(energies, starting_pixel::Int)
	...
	indices = zeros(Int, size(energies)[1])
	indices[1] = starting_pixel
	
	for i in 2:size(energies)[1]
		left, _ = findmax([1, starting_pixel-1])
		right, _ = findmin([size(energies)[2], starting_pixel+1])

		_, best_index = findmin([energies[i, left], energies[i, starting_pixel], energies[i, right]])
		
		best_index = best_index - 2 + starting_pixel
		if best_index == 0
			best_index = 1
		end
		indices[i] = best_index
		starting_pixel = best_index
...
function least_energy(energies, i, j)
	...
	if i == size(energies)[1]
		return (energies[end, j], 0)
	end
	
	left, _ = findmax([1, j-1])
	right, _ = findmin([size(energies)[2], j+1])

	left_energy = least_energy(energies, i+1, left)
	center_energy = least_energy(energies, i+1, j)
	right_energy = least_energy(energies, i+1, right)
	
	min_energy, index = findmin([left_energy[1], center_energy[1], right_energy[1]])
...
function recursive_seam(energies, starting_pixel)
	...
	next_pixel = starting_pixel
	seam = zeros(Int, m)
	
	seam[1] = starting_pixel
	for i in 1:(m-1)
		results = least_energy(energies, i, next_pixel)
		next_pixel = results[2]
		seam[i+1] = next_pixel
	end
...
function memoized_least_energy(energies, i, j, memory)
	...
	left_energy = memoized_least_energy(energies, i+1, left, memory)
	center_energy = memoized_least_energy(energies, i+1, j, memory)
	right_energy = memoized_least_energy(energies, i+1, right, memory)
	...
	min_energy += energies[i, j]
	push!(memory, (i, j) => (min_energy, index))
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	seam[1] = starting_pixel
	for i in 1:(m-1)
		results = memoized_least_energy(energies, i, next_pixel, memory)
		next_pixel = results[2]
		seam[i+1] = next_pixel
	end
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	if i == size(energies)[1]
		return energies[end, j], memory
	end
	
	left, _ = findmax([1, j-1])
	right, _ = findmin([size(energies)[2], j+1])

	left_energy, memory = matrix_memoized_least_energy(energies, i+1, left, memory)
	center_energy, memory = matrix_memoized_least_energy(energies, i+1, j, memory)
	right_energy, memory = matrix_memoized_least_energy(energies, i+1, right, memory)
	...
	min_energy += energies[i, j]
	# push!(memory, (i, j) => (min_energy, index))
	memory[i, j] = min_energy
	return min_energy, memory
end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	for i in 1:(m-1)
		...
		val, index = findmin([left_energy, center_energy, right_energy])
...
function least_energy_matrix(energies)
	energy_matrix = copy(energies)
	
	energy_matrix[end, :] = energies[end, :]
	
	for i in (size(energies)[1] - 1):-1:1
		for j in 1:size(energies)[2]
			left, _ = findmax([1, j-1])
			right ,_ = findmin([size(energies)[2], j+1])
			
			min_energy, _ = findmin([energies[i+1, left], energies[i+1, j], energies[i+1, right]])
			energy_matrix[i, j] = energies[i, j] + min_energy
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i in 1:(m-1)
		...
		_, index = findmin([least_energies[i+1, left], least_energies[i+1, j], least_energies[i+1, right]])
...
begin
	img = load(download("https://cdn.britannica.com/s:800x450,c:crop/43/172743-138-545C299D/overview-Barack-Obama.jpg"))#https://i.imgur.com/4SRnmkj.png"))
	img = decimate(img, 2)
end
...
# ╔═╡ ac91150a-4039-11eb-373e-835619bcab72
img2 = decimate(img, 2)
