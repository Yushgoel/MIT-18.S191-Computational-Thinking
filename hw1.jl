...
begin
	Pkg.add(["Images", "ImageMagick", "Distributions"])
	using Images
	using Distributions
end
...
# ╔═╡ abbbea86-3d6b-11eb-2469-e91d1d4828c9
begin
	a = [6 7; 8 9]
	size(a)
end
...
function matrix_to_vecvec(matrix)
	bigVec = [matrix[:,i] for i in 1:size(matrix,2)]
	return bigVec
end
...
function mean_colors(image)
	...
	return (r / length(image), g / length(image), b / length(image))
end
...
# ╔═╡ 65dd3abe-3de4-11eb-28c3-f3214f00e6ce
length(philip)
...
function blur_1D(v, l)
	...
	for i in 1:length(v)
		sum = 0
		for j in -l:l
			sum = sum + extend(v, j+i)
		end
		v_blurred[i] = sum / length(-l:l)
	end
	return v_blurred
end
...
# ╔═╡ 18af4b42-3e02-11eb-0e16-7bf1d3a63857
@bind l_box Slider(0:100, show_value=true)

# ╔═╡ 09ed9708-3e02-11eb-2356-bdc4a1b867e6
...
# ╔═╡ c7caf8bc-3e0a-11eb-25f7-730d49779fd5
Int(1.0)
...
function gaussian_kernel(n)
	...
	G(x) = 1/sqrt(2π) * exp(-x^2 / 2)
	
	for i in 1:n
		append!(kernel, G(i - 1 - mid))
	end
...
# ╔═╡ 0a3610ea-3eec-11eb-1c34-0773cde82e6a
sqrt(9)
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i in 1:size(M)[1]
		for j in 1:size(M)[2]
			...
			for a in 1:y
				for b in 1:x
					sum = sum + extend_mat(M, Int(i + a - 1 - midY), Int(j + b - 1 - midX)) * K[a, b]
				end
			end
			conv_M[i, j] = sum
		end
