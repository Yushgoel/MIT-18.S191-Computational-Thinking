...
row_col_answer = md"""
Row - Probability that the letter appears first in a combination
Column - Probability that the letter appears second in a combination
"""
...
# ╔═╡ aeab44d2-40fc-11eb-28a6-75da00e039ac
word_counts(emma_words)["Emma"]
...
function completions_cache(grams)
	...
	for i in 1:length(grams)
		...
		if haskey(cache, gram[1:n-1])
			cache[gram[1:n-1]] = [cache[gram[1:n-1]] gram[n]]
		else
			cache[gram[1:n-1]] = [gram[n]]
		end
	end
