...
function euler_integrate(fprime::Function, fa::Number, 
	...
	for i in T
		...
		if length(vals) == 0
			...
			f_of_a = vals[length(vals)]
		end
		push!(vals, euler_integrate_step(fprime, f_of_a, a, h))
...
function euler_SIR(β, γ, sir_0::Vector, T::AbstractRange)
	...
	for i in T
		...
		if length(SIRs) == 0
			...
			curr_SIR = SIRs[length(SIRs)]
		end
		push!(SIRs, euler_SIR_step(β, γ, curr_SIR, h))
	end
	num_steps = length(T)
...
# ╔═╡ 403468f6-468c-11eb-36f4-bbcd3b0ab919
@bind beta_slider Slider(0:0.01:1, show_value=true)

# ╔═╡ 467e378a-468c-11eb-3381-95a5b054035c
@bind gamma_slider Slider(0:0.01:1, show_value=true)

# ╔═╡ 0a095a94-1245-11eb-001a-b908128532aa
sir_results = euler_SIR(beta_slider, gamma_slider, [0.99, 0.01, 0.00], sir_T)#, gamma_slider,
...
Sigmoid Growth for number of infected people.

After long time, seems to stabilize. About 75% are infected, but not all. 
"""
...
answer = md"""
β > γ. Bigger difference the better
"""
...
function gradient_descent_1d_step(f, x0; η=0.01)
	...
	x = x0 - η * finite_difference_slope(f, x0)
	return x
end
...
If value to change x by is pretty much 0, then quit early
"""
...
function gradient_descent_2d(f, x0, y0; η=0.01)
	...
	while abs(x - prev_x ) > 0.0001
		prev_x = x
		prev_y = y
		tmp = gradient_descent_2d_step(f, x, y; η)
		x = tmp[1]
		y = tmp[2]
	end
	return [x, y]
...
function loss_dice(μ, σ)
	...
	index = 1
	for x in dice_x
		loss += (gauss(x, µ, σ) - dice_y[index]) ^ 2
		index += 1
	end
	return loss
end
...
found_μ, found_σ = let
	...
	# your code here
	vals = gradient_descent_2d(loss_dice, 30, 1; η=1)
	vals[1], vals[2]
end
...
function loss_sir(β, γ)
	preds = euler_SIR(β, γ, [0.99, 0.01, 0.0], hw4_T)
	loss = 0
	
	for i in 1:length(preds)
		time_pred = preds[i]
		time_sim = hw4_results[i]
		
		s_loss = (time_pred[1] - time_sim[1])^2
		i_loss = (time_pred[2] - time_sim[2])^2
		r_loss = (time_pred[3] - time_sim[3])^2
...
found_β, found_γ = let
	...
	vals = gradient_descent_2d(loss_sir, 0.03, 0.003; η=0.0000001)
	vals[1], vals[2]
end
