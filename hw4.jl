...
student = (name = "Ayush Goel", kerberos_id = "ayush")
...
# ╔═╡ 93d8f0fc-42b7-11eb-2562-e1e7ed513dc4
@bind N_interactive Slider(1:100000, show_value = true)

# ╔═╡ a89745f2-42b7-11eb-006a-397026c58cf8
...
function mean_plotting()
	N = 10_000
	means = []
	for p in 0.001:0.001:1
		data = do_experiment(p, N)
		append!(means, sum(data)/length(data))
	end
	
	plot(0.001:0.001:1, means)
end

# ╔═╡ b3959a84-42b8-11eb-2200-9dc2a6030a0f
mean_plotting()
...
# ╔═╡ 5861fb54-42c1-11eb-3dde-ebef606eeeac
...
# ╔═╡ 57b70b06-4355-11eb-3081-391f168833bd
...
# ╔═╡ 7bdc4442-435a-11eb-12fc-df65f9e455a1
function stat_frequencies(agents::Vector{Agent})
	counts = Dict()
	counts[0] = 0
	counts[1] = 0
	counts[2] = 0
	...
	for i in 1:n
		counts[Integer(agents[i].status)] += 1/n
	end
	return counts
end
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	p = plot()
	plot!(p, 1:T, mean_S, alpha=0.5, label=nothing)
	plot!(p, 1:T, mean_I, alpha=0.5, label=nothing)
	plot!(p, 1:T, mean_R, alpha=0.5, label=nothing)
	return p
end
...
@bind p_infection Slider(0.001:0.001:1, show_value=true)

# ╔═╡ 690c37b8-4365-11eb-3e2c-8727343d557e
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	for i in 1:T
		...
		for sim in simulations
			col_std_S += (sim.S[i] - mean_S[i])^2
			col_std_I += (sim.I[i] - mean_I[i])^2
			col_std_R += (sim.R[i] - mean_R[i])^2
		end	
		
		append!(std_S, (col_std_S / N)^0.5)
		append!(std_I, (col_std_I / N)^0.5)
		append!(std_R, (col_std_R / N)^0.5)
	end
	
	
	p = plot()
	plot!(p, 1:T, mean_S, alpha=0.5, label=nothing, yerr = std_S)
	plot!(p, 1:T, mean_I, alpha=0.5, label=nothing, yerr = std_I)
	plot!(p, 1:T, mean_R, alpha=0.5, label=nothing, yerr = std_R)
	return p
end
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i in 1:T
		...
		freq = stat_frequencies(agents)

		append!(S_counts, freq[0])
		append!(I_counts, freq[1])
		append!(R_counts, freq[2])
	end
...
# ╔═╡ 6ab380b4-4368-11eb-1b9b-93a88e0c38f0
...
simulations2 = repeat_simulations(100, 1000, Reinfection(p_infection, p_recovery), 20)

# ╔═╡ 4f98b16c-4374-11eb-2558-f1a201d9ed12
let
	p = plot()
